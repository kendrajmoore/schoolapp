...
var generatorTransformer = module.exports = traverser.explicit('generator', {
	...
	whileDoWhileStmt: function(stmt, context) {
        ...
        if (context.generator && (
            ...
            context.generator.withcc(function(callcc) {

				__this.stmt( gen.ifStmt(
					stmt.test,
                ...
                return context.generator.withcc(function(cc) {
					...
					return gen.ifStmt(
						expr.operator == '||' ? gen.not(test) : test, [
							cc( __this.expr(expr.right, context) )
						], [
							cc( test )
						]
					);
        ...
        if (!context.isInYield) {
            ...
            if (aHasYields || bHasYields) {
				...
				return context.generator.withcc(function(cc) {
					...
					return gen.ifStmt(
						expr.test, [
							cc( __this.expr(expr.consequent, context) )
						], [
							cc( __this.expr(expr.alternate, context) )
						]
					);
	...
	withGenerator: function (m, context, fn) {
        ...
        var generator = context.generator = context.topLevelGenerator = {
			...
			yield: function(v) {
                return currBlock.yield(v);
            },
			stmt: function(stmt) {
                return currBlock.stmt(stmt);
            }
		}
...
function generatorBlock (context) {
    ...
    return {
		...
		stmt: function(s) {
            return currBasicBlock.stmt(s);
        },
		yield: function(v) {
            return currBasicBlock.yield(v);
        },
		...
		isEmpty: function() {
            return renderedBlocks.length == 0 && currBasicBlock.isEmpty();
        }
	}
}
...
function basicBlock (context, name, opts) {
    ...
    return {
		...
		renderWithContinuation: function(c, v) {
            ...
            r = yields.reduceRight(
				function(s, y) {
                    return gen.block(y.stmts.concat([
		...
		isEmpty: function() {
            return stmts.length + yields.length == 0;
        }
	}
}
...
function genRet() {
    ...
    // Stops the return stmt being transformed
